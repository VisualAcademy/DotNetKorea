...
{
    public interface ISupportRepository : IBreadShop<ArticleBase>
    {
        ...
        List<ArticleBase> GetAll(string userName, int pageNumber, int pageSize = 10);

        /// <summary>
        /// 검색 결과 리스트 
        /// </summary>
        List<ArticleBase> GetSearchAll(string searchField, string searchQuery, int pageNumber, int pageSize = 10);
        List<ArticleBase> GetSearchAll(string userName, string searchField, string searchQuery, int pageNumber, int pageSize = 10);
